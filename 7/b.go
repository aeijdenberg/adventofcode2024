...
func checkIt(answer int, nums []int) int {
	...
	if answer >= nums[len(nums)-1] {
		sL := strconv.Itoa(answer)
		sR := strconv.Itoa(nums[len(nums)-1])
		if sL != sR {
			if strings.HasSuffix(sL, sR) {
				rv := checkIt(MustInt(sL[:len(sL)-len(sR)]), nums[:len(nums)-1])
				if rv != 0 {
					return answer
				}
			}
